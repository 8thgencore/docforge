...
export const TagsPage = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (params: { tagId: string; name: string }) => api.updateTag(config, params.tagId, { name: params.name }),
    onSuccess: async () => {
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid gap-4">
          <p className="text-muted-foreground text-sm">{t("tags.pageHint")}</p>
          ...
          <div className="space-y-2">
            {tagsQuery.data?.map((tag) => (
              <div key={tag.id} className="border-border rounded-md border p-3">
                {editingId === tag.id ? (
            ...
            {!tagsQuery.data?.length && <p className="text-muted-foreground text-sm">{t("tags.empty")}</p>}
          </div>
        </div>
      </CardContent>
    </Card>
