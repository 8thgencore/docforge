...
def test_scope_filter_none_when_empty() -> None:
    assert build_scope_filter(group_id=None, category=None) is None


def test_scope_filter_contains_group_and_category() -> None:
    group_id = uuid.uuid4()
    q_filter = build_scope_filter(group_id=group_id, category="reports")

    assert q_filter is not None
    assert q_filter.must is not None
    assert len(q_filter.must) == 2
    assert q_filter.must[0].key == "group_id"
    assert q_filter.must[1].key == "category"
